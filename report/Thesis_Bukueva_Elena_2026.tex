...
enabling direct comparison across frameworks and hardware targets. Roofline analysis classifies per-layer execution regimes, and GPU kernel-level profiling with Nsight Systems
...
expanded. The Norwegian summary has not yet been completed. Parts of the results chapter still require restructuring
and refinement. In addition, some figures, tables, references, appendices, and formatting elements may still
contain placeholders, inconsistencies, or rendering artifacts. Final revision of the text, structure, and presentation
will be completed during the concluding phase of the thesis work.}

\begin{otherlanguage}{norwegian}
\chapter*{Sammendrag}
\addcontentsline{toc}{chapter}{Sammendrag}

Effektiv utrulling av dyplæringsmodeller på kantmaskinvare er ikke utelukkende et spørsmål om modellstørrelse.
Inferensytelse bestemmes av samspillet mellom modellarkitektur, numerisk presisjon og i hvilken grad
inferensrammeverket kan utnytte målmaskinvaren.
Denne avhandlingen presenterer en systematisk, målebasert studie av kompilatorbevisst optimalisering for kant-AI,
med YOLOv8n objektdeteksjon på to representative kantplattformer --- NVIDIA Jetson Orin Nano og Raspberry Pi~5 ---
som konkret kasusstudie.

Et bredt spekter av optimaliseringsstrategier evalueres gjennom fem inferensrammeverk
(TensorRT, Apache TVM med MetaSchedule-autotuning, ONNX Runtime, Google LiteRT og \texttt{torch.compile}),
to presisjonsnivåer (FP16 og INT8), strukturell kanalbeskjæring med ettertrening,
og substitusjon av aktiveringsfunksjon.
Det eksperimentelle opplegget inkluderer isolerte kjøringer i egne delprosesser, oppvarmingskjøringer,
konfidensintervaller og strømmålinger for hvert benkemål,
noe som muliggjør direkte sammenligning på tvers av rammeverk og maskinvareplattformer.
Roofline-analyse brukes til å klassifisere kjøreregimer per lag, og GPU-kjerneprofil\-ering med Nsight Systems
brukes til å dekomponere TensorRT FP32-motoren i konkrete overheadkategorier.
Profileringen viser at kun 59\,\% av motortiden brukes på nyttige konvolusjonsberegninger.
De resterende 41\,\% fordeler seg på tre identifiserbare kilder: usammenslåtte SiLU-aktiveringer (19\,\%),
eksplisitte minnekopieringsnoder ved C2f-kanalsplitgrensene (6{,}7\,\%), og interne NCHW$\to$kCHW32-formatkonverteringer
i C2f-undergrafen (17\,\% av ryggradets konvolusjonstid).
Disse funnene motiverte utviklingen av en tilpasset TensorRT-plugin som erstatter hele C2f-undergrafen med én enkelt
ugjennomsiktig node og eliminerer formatgrensen som standard eksport ikke kan løse automatisk.
Pluginen fjernet den strukturelle overheaden, men gevinsten ble motsvart av tapet av TensorRTs Tensor Core-kjerner,
slik at den totale inferenstiden ikke ble forbedret sammenlignet med standardmotoren.

Resultatene avdekker flere ikke-åpenbare funn.
På Raspberry Pi~5 oppnår LiteRT INT8 31\,ms inferenstid, en $9{,}4\times$ reduksjon sammenlignet med
290\,ms for PyTorch eager-basislinjen, noe som viser at riktige utrullingsvalg betyr langt mer enn
arkitektoniske endringer for denne klassen maskinvare.
TVM MetaSchedule var det eneste rammeverket som oppnådde ekte FP16-akselerasjon på ARM-CPU og
reduserte ventetiden til 122\,ms, mens alle andre rammeverk enten ble tregere eller feilet
på grunn av manglende halvpresisjonskjerner.
På Orin Nano oppnådde TVM i FP16 12{,}5\,ms gjennom WMMA-tensorisering, noe som er 26\,\% raskere
enn ONNX Runtime og nærmer seg TensorRT (6{,}3\,ms).
Profileringen avslørte videre at SiLU-aktiveringer ikke sammenslås med konvolusjoner av TensorRT, i motsetning
til ReLU, et funn med direkte implikasjoner for arkitekturvalg på ressursbegrensede plattformer.
Strukturell kanalbeskjæring viste seg å være ineffektivt som frittstående strategi: et systematisk søk
gjennom 298 lagkonfigurasjoner fant ingen enkeltbeskåret modell med mer enn $1{,}04\times$ hastighetsøkning,
og det beste flerl\-ags\-resultatet med ettertrening ga kun 2{,}3\,\% redusert ventetid til en kostnad
av 4{,}0\,\% lavere mAP50-95.
Dette bekrefter at kanalreduksjoner ikke gir proporsjonal tidsgevinst når cuDNNs kjernekornstørrelse
er grovere enn de anvendte reduksjonene.

Samlet sett viser resultatene at valg av inferensrammeverk, presisjonsstrategi og maskinvaretilpasning
på lavt nivå --- ikke modellstørrelsen alene --- er de dominerende faktorene for inferenseffektivitet
på kantmaskinvare.
...
The custom TensorRT plugin is developed and evaluated for the C2f block at model.2 (32-channel, 160$\times$160 spatial
resolution).
...
\item \textbf{Chapter~5 - Testing and Results} presents the experimental results across all optimization strategies:
...
kernels. For convolution in particular, many implementations reduce the operation to an implicit General Matrix
Multiplication (GEMM)-like form, which
...
throughput. For inference, this is highly relevant: the theoretical gain from FP16, BF16, TensorFloat-32 (TF32), or INT8 is achieved
...
and should be treated as hardware- and software-specific rather than portable. Third, the user has only partial
visibility into why a particular tactic was chosen,
...
%
%Deploying neural networks efficiently requires understanding where and why execution time is
%lost. Bottlenecks in ML inference arise from an interplay of memory architecture, data layout,
%compiler decisions, and workload scheduling. This section surveys the main categories of
%performance limitations encountered in practice and explains the mechanisms behind each.
%
%A workload is compute-bound when execution time scales with the number of
%multiply-accumulate (MAC) operations and shrinks proportionally with added arithmetic
%throughput. It is memory-bound when the processor stalls waiting for data rather than
%computing. The Roofline model, introduced by Williams et al.~\cite{williams2009roofline},
%provides a concise way to diagnose which regime applies by relating an operation's
%arithmetic intensity (FLOPs per byte of memory traffic) to the hardware's peak
%compute-to-bandwidth ratio.
%
%The classical algorithmic response to memory pressure is blocking (also called
%tiling): partition the matrices $A$, $B$, and accumulator $C$ into sub-tiles that fit
%in L2 or L1 cache, so each element is reused many times before being evicted.  A
%multiply-accumulate step then becomes:
%\[
%  C[i,j] \mathrel{+}= \sum_{k} A[i,k] \cdot B[k,j],
%\]
%where the tile sizes are chosen to maximise reuse for a specific cache capacity. Compute-in-memory
%(CIM) proposals carry this principle further by performing the multiply-accumulate operations
%directly inside the memory array, eliminating the data-movement cost
%entirely~\cite{emer_sze_chen_2019_dnn_accel_tutorial}.
%
%\subsection{Memory Layout: NCHW vs.\ NHWC}
%
%The physical order in which tensor elements are stored in memory has a significant effect on
%vectorisation efficiency and cache utilisation. Two conventions dominate deep learning
%frameworks: NCHW (batch, channels, height, width), where the channel dimension is
%innermost after the batch axis, and NHWC (batch, height, width, channels), where
%spatial positions group together.
%
%For convolutions on NVIDIA GPUs with Tensor Cores, NHWC is typically preferred because Tensor Core convolution kernels
%operate most efficiently when the channel dimension is the fastest-varying dimension in memory. NVIDIA documents that
%Tensor Core convolutions require NHWC layout internally and are fastest when input tensors are already stored as NHWC.
%NCHW tensors can still use Tensor Cores, but cuDNN/TensorRT may insert NCHW-to-NHWC transformations, whose overhead
%can be measurable, especially when layout conversion cost is large relative to the convolution itself.
%
%On CPUs, oneDNN often prefers blocked memory formats such as nChw8c or nChw16c, where the channel dimension is split
%into SIMD-sized blocks. This lets convolution kernels operate on vectors efficiently and improves cache locality.
%If the channel count is not a multiple of the block size, oneDNN may use zero-padding in the blocked dimension so
%kernels can avoid special tail-handling code~\cite{onednn_memory_formats, onednn_memory_format_tag}.
%
%Layout mismatches across framework boundaries are a common source of latency. A model
%exported from PyTorch in NCHW format and consumed by a TensorRT engine that internally
%prefers NHWC will incur a transpose operation at each I/O boundary. Compilers such as TVM
%and TensorRT insert explicit layout-conversion nodes during graph compilation to ensure
%internal uniformity, but when mixed-precision layers with different layout requirements
%appear adjacently, the conversion overhead can dominate a short operator's runtime. Profiling
%for unnecessary transposes is therefore an important step in deployment
%optimisation~\cite{chen2018tvm}.
%
%\subsection{Tensor Transformations for Efficient Convolution}
%
%Direct computation of a convolution requires $O(K^{2} \cdot C_{in} \cdot C_{out} \cdot H
%\cdot W)$ multiplications. Two classical transformations reduce this cost by restructuring
%the computation.
%
%\paragraph{im2col.}
%The im2col transformation unfolds each receptive field of the input feature map into a
%column of a matrix, converting the convolution into a single general matrix multiplication
%(GEMM). This allows highly tuned BLAS libraries to be applied directly. The trade-off is
%memory: unfolding a $K \times K$ receptive field over $C_{in}$ channels creates a matrix
%with $K^{2} C_{in}$ rows, increasing the memory footprint by a factor of $K^{2}$ compared
%to the original activation tensor. For large kernels or high-resolution feature maps, this
%expansion can stress the cache hierarchy and reduce the arithmetic-intensity gains expected
%from GEMM.
%
%\paragraph{Winograd's algorithm.}
%The Winograd minimal filtering algorithm reduces the number of multiplications required for
%small convolutions (typically $3\times3$ with stride 1) from $K^{2}$ to as few as
%$(K+r-1)^{2}/r^{2}$ multiplications per output tile of size $r$, at the cost of additional
%additions and a change of basis. For the common $F(2\times2, 3\times3)$ case the number of
%multiplications drops from 36 to 16 per $2\times2$ output tile, a 2.25$\times$ reduction.
%Liu et al.~\cite{liu2021optimizing} demonstrate that aligning the Winograd basis transformation
%with Tensor Core tile sizes can yield further speed-ups on modern GPUs. A combined
%Im2col-Winograd approach~\cite{zhang2024im2col} decomposes an $N$-dimensional convolution
%into a sequence of 1-D Winograd transforms applied along each spatial axis, avoiding the
%memory explosion of standard im2col while retaining compatibility with NHWC layout. Reported
%speed-ups over cuDNN reach up to $2.05\times$ for certain configurations on contemporary
%NVIDIA hardware.
%
%The Winograd algorithm introduces numerical instability at larger tile sizes, so it is
%typically restricted to $3\times3$ and $5\times5$ kernels and disabled for depthwise
%separable convolutions, where its arithmetic savings are smaller.
%
%\subsection{Operator Fusion and Its Limitations}
%
%Operator fusion merges adjacent computation nodes in the execution graph into a single
%compiled kernel, eliminating the round-trip to global memory that would otherwise be required
%between them. The performance benefit is straightforward: if a convolution is followed by
%batch normalisation and a ReLU activation, executing them as three separate kernels requires
%writing intermediate feature maps to DRAM and reading them back twice. A fused kernel keeps
%these intermediate values in registers or shared memory, improving both memory traffic and
%kernel-launch overhead.
%
%Frameworks such as TensorRT, TVM, and XLA apply fusion aggressively during graph
%compilation~\cite{nvidia_tensorrt_dev_guide, chen2018tvm, frostig2019compiling}. However,
%fusion is not always possible, and failed fusion silently degrades performance.
%
%A representative example is the Swish/SiLU activation, defined as $\sigma(x) = x
%\cdot \text{sigmoid}(x)$. Because SiLU requires a higher-precision intermediate value for
%the sigmoid computation to maintain accuracy, TensorRT does not always fuse
%it with a preceding INT8-quantised convolution: the precision mismatch forced an explicit
%dequantise-compute-requantise sequence, inserting two memory transactions where a fused
%kernel would have required none~\cite{nvidia_tensorrt_docs}. A similar issue arises with
%GELU and its approximations in transformer blocks: the $\tanh$ component is not
%directly representable in tensor-core arithmetic, so compilers frequently emit it as a
%separate scalar kernel, breaking the fusion chain from the preceding linear projection.
%
%Beyond specific operators, custom or user-defined operations that fall outside a compiler's supported operator set are
%unconditionally excluded from fusion, creating a guaranteed memory bottleneck wherever such
%operations appear in a graph. Profiling tools such as Nsight Systems expose these gaps as
%short, rapidly repeating kernel launches with high idle time between them.
%
%\subsection{Ahead-of-Time vs.\ Just-in-Time Compilation}
%
%The timing of when a model graph is lowered to machine code fundamentally affects both
%startup behaviour and peak throughput.
%
%\paragraph{Ahead-of-Time (AOT) compilation}
%translates the model to a static binary during a build phase, before any inference request
%arrives. Because compilation time is not charged to the end user, the compiler can afford
%exhaustive search strategies such as evolutionary auto-tuning, polyhedral scheduling, and
%multi-level tiling optimisation that would take minutes or hours to run. The resulting binary
%has deterministic latency from the very first inference call, which is critical in real-time
%systems where a single slow startup frame is unacceptable. TVM's \path{tvm compile} and
%TensorRT's engine serialisation both follow the AOT model. The downside is rigidity:
%a serialised engine is tied to a specific batch size, input resolution, and hardware target;
%any change requires a full recompilation.
%
%\paragraph{Just-in-Time (JIT) compilation}
%defers lowering to the moment a graph is first executed, using live information about actual
%tensor shapes, data types, and hardware state. PyTorch's \path{torch.compile} backend and
%TVM's dynamic shape support are JIT frameworks. JIT compilation can adapt to variable-length
%inputs, dynamically chosen batch sizes, and runtime-detected hardware features that an AOT
%compiler cannot anticipate at build time. Jia et al.~\cite{jia2024empowering} show that a
%JIT kernel auto-generation system for in-browser edge inference achieves up to $8.2\times$
%faster throughput within 30 seconds of first execution compared to pre-compiled baselines
%on the same device, because it generates kernels precisely matched to the browser's WebGPU
%backend. The cost is a warm-up latency: the first call to a JIT-compiled model is
%substantially slower than subsequent calls, and in latency-sensitive pipelines this cold-start
%penalty may be unacceptable.
%
%In practice, edge deployments almost universally prefer AOT: the target hardware is fixed,
%the input resolution is known at design time, and the absence of a runtime compiler reduces
%memory footprint. JIT strategies are more common in server-side or research settings where
%flexibility outweighs determinism.
%
%\subsection{Batching: Throughput vs.\ Latency}
%
%Neural network inference throughput increases with batch size because larger batches amortise
%fixed overheads - kernel launch costs, memory-allocation calls, and pipeline-fill latency -
%across more output samples. On an NVIDIA edge GPU, batched inference can process up to
%$2.4\times$ more inferences per second compared with single-sample execution when the
%hardware's compute units would otherwise be underutilised~\cite{hao2023reaching}.
%
%However, batching fundamentally conflicts with latency requirements. A system that waits to
%accumulate a batch of $B$ samples before dispatching inference will add a queuing delay
%proportional to $B$ times the inter-arrival time of requests. In real-time applications such
%as object detection in a live video stream, latency measured end-to-end includes this waiting
%time, so large batches are counter-productive. The Sarathi-Serve
%scheduler~\cite{agrawal2024taming} addresses this tension in LLM serving by splitting prefill
%requests into equal-sized chunks that can be interleaved with ongoing decode batches, achieving
%near-peak throughput without stalling low-latency decode streams. A simpler version of the
%same principle applies to edge vision pipelines: processing frames one at a time preserves
%temporal responsiveness, while offline evaluation tasks (e.g., dataset benchmarking) should
%use the largest batch that fits in device memory.
%
%For deeply memory-bound workloads, batching also raises arithmetic intensity and can push
%the operating point above the Roofline's bandwidth ceiling, transforming a memory-bound
%regime into a compute-bound one. This transition is itself an argument for profiling at
%multiple batch sizes when characterising a deployment target.
%
%\subsection{Underutilisation of Compute Resources}
%
%High peak compute throughput does not imply high utilised throughput. Several mechanisms
%cause the hardware's arithmetic units to sit idle even when inference is actively running.
%
%\paragraph{Low GPU/NPU occupancy.}
%GPU execution is organised into warps of 32 threads scheduled onto streaming multiprocessors
%(SMs). A kernel with insufficient parallelism - for example, a $1\times1$ convolution with
%a very small feature map - may launch fewer warps than the number of SMs, leaving most of
%the chip idle. This is common in the deeper layers of a network where spatial resolution
%has been reduced by pooling but the design has not increased channel count enough to
%compensate.
%
%\paragraph{Memory-bandwidth-induced compute stalls.}
%When operators are strongly memory-bound, arithmetic units stall waiting for cache misses to
%be resolved. The processor appears busy from a utilisation-monitoring perspective but is
%not executing useful arithmetic. This is the hardware manifestation of the memory-wall
%problem described in Section~\ref{sec:mem_bottleneck}, and profiling tools distinguish it
%by reporting a high ratio of memory-bound stall cycles to total active cycles.
%
%\paragraph{Small and fragmented kernel launches.}
%Compilers that fail to fuse consecutive lightweight operators emit many short kernels in
%rapid succession. Each kernel launch incurs a fixed overhead on the order of a few
%microseconds, and for operations whose compute time is shorter than this overhead - such as
%channel-wise bias additions, scalar multiplications, or simple element-wise activations -
%the dispatch cost dominates execution time. Nsight Systems traces of unoptimised models
%frequently reveal hundreds of sub-microsecond kernels occupying only a fraction of the
%timeline while GPU utilisation reads low~\cite{nvidia_tensorrt_dev_guide}.
%
%\paragraph{Data-type and precision mismatches.}
%Tensor Core hardware requires inputs to be aligned to specific multiples of the tile
%dimension (typically multiples of 8 or 16 for INT8/FP16). When layer dimensions do not
%satisfy this constraint - common in custom attention heads with non-power-of-two hidden
%sizes - the compiler pads the tensors, and the padded elements consume bandwidth and compute
%cycles without contributing to the result. Selecting model dimensions that are
%hardware-aligned is therefore a design-time decision with measurable inference-time
%consequences~\cite{markidis2018tensorcores}.
...
that FP16 enabled tensorisation through Warp Matrix Multiply-Accumulate (WMMA)-style
matrix operations on the Orin Nano GPU. In the FP32 campaign, no task could be mapped
...
128-bit NEON (ARM Advanced SIMD extension) register with eight \path{float16} values instead of four
...
Nsight Systems profiling was performed on the TensorRT FP32 engine for YOLOv8n on the Jetson Orin Nano
(batch\,=\,1, 640$\times$640 input, raw inference without NMS post-processing).
Each session used three warm-up passes to allow CUDA JIT compilation to complete before timing,
followed by five timed runs.
The kernel execution trace extracted from the profiling session shows that each inference cycle
comprises 167 TRT inference kernels plus 3 device-to-host copy kernels for the output tensor,
totalling 170 GPU kernel launches per cycle.
The first warm-up run completed in 44.4\,ms due to CUDA JIT compilation of all kernel variants on
first call, an 8.8$\times$ inflation over steady state, illustrating why multiple warm-up passes are
essential before timing.
The steady-state GPU execution time, taken as the median NVTX \path{ExecutionContext::enqueue}
duration across the five timed runs, was 4.84\,ms per inference.
The Python-level wall-clock median from this session was 17.97\,ms, measured with only 5 timed runs
and 3 warm-up iterations using \path{tensorrt\_evaluation\_indiv.py}.
The main benchmark results reported in Table~\ref{tab:orin_backend_benchmark} were produced by
\path{tensorrt\_evaluation\_dual.py} with 200 timed runs and 30 warm-up iterations per repetition
across 5 isolated repetitions, which yields a more stable estimate; those figures (11.38\,ms mean)
should be treated as the authoritative latency reference.
The nsys session here is used exclusively for kernel-level decomposition, not for
