...
\begin{titlepage}
    ...
    \begin{tabular}{@{}p{4.8cm}p{13.2cm}@{}}
        ...
        \textbf{Thesis description:}     & \ThesisDescription \\[0.5cm]
        \textbf{The tasks will be:}      & \\[-0.2cm]
        \multicolumn{2}{@{}l@{}}{%
          \hspace{4.8cm}%
...
TensorRT does fuse the Sigmoid and the element-wise multiply into a single pointwise node
(\path{PWN}), avoiding one kernel launch per activation, but it does not merge the activation
...
Prior work on alternative activation functions in YOLO architectures suggests that
fine-tuning for 10–30 epochs can largely recover the accuracy gap, but a full retraining
from scratch is required to match the original mAP50 within 0.3--0.5\,\%.
