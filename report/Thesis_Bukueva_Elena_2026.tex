...
passes through the Python interpreter; the Global Interpreter Lock (GIL) limits parallel execution; tensors
and operators are heap-allocated Python objects; and each kernel dispatch requires the framework to resolve
...
NPUs, and FPGAs. These runtimes execute pre-compiled computation graphs, call highly optimized kernels
...
A further challenge is that standard kernel libraries such as cuDNN and oneDNN were primarily designed for
throughput-oriented, large-batch training workloads. Large batches improve cache efficiency by reusing
weights across many samples and provide enough parallel work to justify complex tiling and scheduling
strategies in GEMM-based kernels. At batch size~1, which is standard for real-time edge inference, these
assumptions break down: memory access patterns become less regular, kernel occupancy drops, and launch
overhead contributes a larger share of total latency.
...
An ML model compiler takes a high-level model representation — expressed in a framework such as PyTorch,
TensorFlow, or as an ONNX graph — and translates it into efficient executable code for a specific target
hardware (CPU, GPU, NPU, FPGA, or a dedicated ASIC). This translation process is orchestrated by a
compiler and runtime stack, illustrated in Figure~\ref{fig:compflow}.
...
Compilation proceeds through several stages. The frontend ingests the model from its source
framework (PyTorch, TensorFlow, or JAX), typically via an exported intermediate format such as ONNX,
TorchScript, or StableHLO, and converts it into the compiler's internal representation (IR) while
normalizing operator variants and control flow. The graph optimization stage then applies
framework-agnostic transformations: operator fusion and simplification, constant folding, dead-branch
...
\begin{itemize}
    \item \textbf{TensorFlow/XLA}, which compiles TensorFlow programs through XLA using HLO-based
        intermediate representations~\cite{tensorflow_xla_architecture,xla_hlo_tooling};
    \item \textbf{PyTorch 2.x}, which combines TorchDynamo, AOTAutograd, and TorchInductor to capture
        and optimize models for efficient execution~\cite{pytorch_torch_compile,pytorch_compile_faq,pytorch_2x};
    \item \textbf{Apache TVM}, a compiler stack for optimizing models across heterogeneous targets
        including CPUs, GPUs, and edge-oriented
        accelerators~\cite{apache_tvm_home, chen2018tvm};
    \item \textbf{MLIR-based stacks}, such as IREE and TensorFlow MLIR, which use multi-level
        intermediate representations to enable flexible lowering pipelines for heterogeneous
        systems~\cite{mlir_overview,tensorflow_mlir_overview,iree_home};
    \item \textbf{ONNX Runtime}, a cross-platform inference engine that supports hardware-specific
        execution providers for efficient deployment on diverse
        platforms~\cite{onnxruntime_home,onnxruntime_execution_providers}.
\end{itemize}
...
\section{Pruning and structural simplification}
    ...
    pruning is better than one-shot pruning (cutting the proportion of weights in one cycle).
    ...
    Another question that arises is whether these subnetworks are transferable to other tasks.
    ...
    \begin{itemize}
        ...
        \item \textbf{Data independent pruning methods}. Rely on the assumption that properties of the
            ...
            while achieving 2-3\% higher Top-1/Top-5 accuracy than the original VGG-16 model.
...
workloads~\cite{nvidia_cudnn_frontend, cutlass_overview, cutlass_implicit_gemm, jia2018volta, yan2020tensorcores}.
...
optimization~\cite{nvidia_tensorrt_docs, nvidia_tensorrt_plugins, nvidia_tensorrt_engine_tools, nvidia_tensorrt_cmd,
    nvidia_cudnn_frontend}. This layered view is especially useful in a compiler-aware thesis because it clarifies
...
throughput~\cite{markidis2018tensorcores, jia2018volta, yan2020tensorcores, sun2022dissecting_tensor_cores}. For
inference, this is highly relevant: the theoretical gain from FP16, BF16, TF32, or INT8 is achieved only when the
surrounding kernel implementation can sustain data movement and feed the compute units efficiently. TensorRT's
tactic-selection mechanism exists precisely to navigate this design space on a concrete target
device~\cite{nvidia_tensorrt_perf, nvidia_tensorrt_support_matrix}.
...
~\cite{nvidia_tensorrt_quickstart, nvidia_tensorrt_support_matrix}. This means that deployment success depends not
...
releases, custom layers are implemented through the \texttt{IPluginV3} interface, while earlier plugin APIs are
deprecated~\cite{nvidia_tensorrt_plugins_cpp, nvidia_tensorrt_plugin_migration}. A plugin declares its inputs, outputs,
...
includes it as a first-class component during engine building
~\cite{nvidia_tensorrt_plugins, nvidia_tensorrt_plugin_migration, nvidia_onnx_graphsurgeon,
    nvidia_tensorrt_plugin_advanced}. This is the cleanest route for injecting a hand-written CUDA implementation
into an otherwise TensorRT-optimized model.
...
plan~\cite{nvidia_tensorrt_plugins, nvidia_tensorrt_plugin_migration, nvidia_tensorrt_cmd}. This mechanism is narrow
...
\texttt{trtexec} can expose layer information, per-layer runtimes, and selected engine structure via flags such
as \texttt{--profilingVerbosity=detailed}, \texttt{--dumpLayerInfo}, and
\texttt{--dumpProfile}~\cite{nvidia_tensorrt_engine_tools, nvidia_tensorrt_cmd}. While these tools do not reveal full
...
kernel ecosystem~\cite{onnxruntime_docs, onnxruntime_cuda_ep, onnxruntime_trt_ep}. This distinction is analytically
...
and should be treated as hardware- and software-specific rather than portable~\cite{nvidia_tensorrt_support_matrix,
    nvidia_tensorrt_glossary}. Third, the user has only partial visibility into why a particular tactic was chosen,
...
approaches~\cite{rammer2020, roller2022, mononn2024}. This observation helps explain why even TensorRT-optimized models
...
\path{torch.compile}. As in the previous benchmark sections, the comparison
...
Latency results, shown in Table~\ref{tab:int8_latency_comparison}, reveal an equally important deployment trade-off.
The fastest configuration in the measured benchmark was TensorRT implicit INT8 quantization, achieving approximately
5.34\,ms mean latency and 187.23 FPS. This is a substantial speedup over both PyTorch FP32 and ONNX Runtime INT8 QDQ.
ONNX Runtime INT8 QDQ, despite using an INT8 graph, was actually slower than the PyTorch FP32 baseline in the
presented experiment, reaching approximately 39.82\,ms per inference. TensorRT FP16, discussed in the previous section,
already provides a strong optimized reference at 6.31\,ms, which means that the practical value of INT8 must be judged
against both accuracy loss and the already highly optimized higher-precision TensorRT paths. The QDQ-based TensorRT
engine reached 7.71\,ms, which is still faster than PyTorch and ONNX Runtime, but notably slower than the best
TensorRT implicit INT8 result.
